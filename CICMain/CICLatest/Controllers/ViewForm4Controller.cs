...
namespace CICLatest.Controllers
{
    ...
    public class ViewForm4Controller : Controller
    {
        ...
        public ViewForm4Controller(IMemoryCache memoryCache, AzureStorageConfiguration azureConfig, ApplicationContext context, UserManager<UserModel> userManager)
        {
            ...
            if (isExist)
            {
                ...
                switch (model.Reviewer)
                {
                    ...
                    case "Ops Manager":
                        ...
                        for (int i = 0; i < cntJson; i++)
                        {
                           model.RegistrationID = view1Form.UpdateRegistrationDetails(myJObject, i, invoiceno, Convert.ToDecimal(model.RegistrationFee), Convert.ToDecimal(model.AdminFee), Convert.ToDecimal(model.RenewalFee), model.PostalAddress,model.TradingStyle, Convert.ToDecimal(penalty));
                        }
                        break;
                }
